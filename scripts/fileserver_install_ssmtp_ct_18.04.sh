...
section "Setting SSMTP Server Variables"
...
# Testing emailing
section "File Server CT - Testing your SSMTP Configuration."
echo
...
if [ $(grep -q "smtp_port=465" /etc/webmin/mailboxes/config; echo $?) -eq 1 ]; then
...
if [ $(grep -q "smtp_user=$SSMTP_AUTHUSER" /etc/webmin/mailboxes/config; echo $?) -eq 1 ]; then
...
if [ $(grep -q "smtp_pass=$SSMTP_AUTHPASS" /etc/webmin/mailboxes/config; echo $?) -eq 1 ]; then
...
if [ $(grep -q "smtp_auth=Login" /etc/webmin/mailboxes/config; echo $?) -eq 1 ]; then
